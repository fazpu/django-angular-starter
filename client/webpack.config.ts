...
const E2E = EVENT.includes('e2e-spec');
...
const clientConfig = function webpackConfig(): WebpackConfig {
  ...
  config.module = {
    rules: [
      ...
      },
      {
        ...
        exclude: [/\.(spec|e2e|e2e-spec|d)\.ts$/]
      },
  ...
  // added
  const base_config_output = config.output = {
      path: root('../server/static/dist/client'),
  ...
  // added
  if (!DLL && !WATCH) {
    config.output = base_config_output;
  } else if (!DLL && WATCH) {
    config.output = {
      ...base_config_output,
